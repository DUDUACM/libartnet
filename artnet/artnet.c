...
// Bit masks for port_status field, shared between GoodInput and GoodOutputA.
// Bits 7,6,5,4 have identical meaning for both input and output ports.
// Bits 3,2,1 differ by port direction (output / input):
//   Bit 3: PORT_STATUS_MERGE (output=merging) / PORT_STATUS_INPUT_DISABLED (input=disabled)
//   Bit 2: unused (output) / unused (input) -- both transmit as zero
//   Bit 1: PORT_STATUS_LPT_MODE (output=LTP merge) / unused (input)
uint8_t PORT_STATUS_LPT_MODE = 0x02;       // bit 1 (output): LTP merge mode active
uint8_t PORT_STATUS_MERGE = 0x08;          // bit 3 (output): output is merging data
uint8_t PORT_STATUS_INPUT_DISABLED = 0x08; // bit 3 (input): input is disabled (same bit as MERGE)
...
int artnet_start(artnet_node vn) {
  ...
  // wait for ArtPoll before replying. reply_addr stays 0 until set by handle_poll.
  return ret;
}
...
/**
 * Send an ArtPoll to the specified ip, or if null, will broadcast.
 *
 * @param vn the artnet_node
...
int artnet_send_nzs(artnet_node vn,
  ...
  if (length < 1 || length > ARTNET_DMX_LENGTH) {
    artnet_error("%s : Length of data out of bounds (%i < 1 || %i > 512)", __FUNCTION__, length);
    return ARTNET_EARG;
  }

  uni &= 0x7FFF;

  // ARTNET_SRV: send through matching input port (uses subscriber unicast)
  if (n->state.node_type == ARTNET_SRV) {
    for (i = 0; i < ARTNET_MAX_PORTS; i++) {
      if (n->ports.in[i].port_enabled && n->ports.in[i].port_addr == uni) {
        return artnet_tx_nzs(n, i, start_code, length, data);
      }
    }
    artnet_error("%s : No input port bound to universe %u", __FUNCTION__, uni);
    return ARTNET_EARG;
  }
 ...
 * @param vn       the artnet_node
 * @param e        the node entry to address
 * @param settings per-port settings bitmask
 * @return ARTNET_EOK on success, or a negative error code
 */
...
/**
 * Get the last received NZS start code for a port.
 *
 * @param vn      the artnet_node
 * @param port_id the port index to query
 * @return the last NZS start code, 0 if none, or negative on error
 */
int artnet_get_nzs_start_code(artnet_node vn, int port_id) {
  node n = (node) vn;
  ...
  return (int)n->ports.out[port_id].nzs_start_code;
}
...
/**
 * Set the Status3 field for the node (ArtPollReply Status3, Art-Net 4).
 *
 * @param vn      the artnet_node
 * @param status3 the Status3 byte value (ARTNET_STATUS3_* bits)
 * @return ARTNET_EOK on success, or a negative error code
 */
int artnet_set_status3(artnet_node vn, uint8_t status3) {
  node n = (node) vn;
  check_nullnode(vn);
 ...
 *
 * @param vn       the artnet_node
 * @param port_id  the physical port number (0 to ARTNET_MAX_PORTS-1)
...
/**
 * Return a pointer to the statically allocated error string.
 *
 * @return a pointer to the last error string (do not free)
 */
...
/**
 * Update the node list with a new ArtPollReply entry.
 *
 * @param n     the node
 * @param nl    the node list to update
 * @param reply the ArtPollReply packet
 * @return ARTNET_EOK on success, or ARTNET_EMEM on allocation failure
 */
 ...
 *
 * @param e     the node entry to populate
 * @param reply the ArtPollReply packet
 */
void copy_apr_to_node_entry(artnet_node_entry e, artnet_reply_t *reply) {
...
/**
 * Find a node_entry in the node list by IP address.
 *
 * @param n the node
 * @param e the node entry to search for (matched by IP)
 * @return a pointer to the private entry, or NULL if not found
 */
...
/**
 * Check for various timeouts: DMX fail-safe, ArtSync, node list cleanup,
 * firmware transfer, DMX keepalive retransmission, and pending ArtPollReply.
 *
 * @param n the node
 */
