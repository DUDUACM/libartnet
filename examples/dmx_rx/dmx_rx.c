/*
 * dmx_rx.c
 * Art-Net 4 DMX receiver example (4-universe)
 *
 * Receives DMX on 4 universes (single node) and prints channel values.
 *
 * Usage: dmx_rx [-i <bind_ip>] [-n <net>] [-s <subnet>] [-u <universe>]
...
/**
 * Print ArtSync notifications.
 *
 * @param n    the artnet_node
 * @param pp   unused packet pointer
...
/**
 * Entry point for the DMX receiver example.
 *
...
int main(int argc, char *argv[]) {
  ...
  printf("Art-Net 4 DMX Receiver (%d universes)\n", NUM_PORTS);
  printf("  Bind IP  : %s\n", ip ? ip : "auto");
  ...
  artnet_set_short_name(node, "LibArtNet DMX-RX");
  artnet_set_long_name(node, "libartnet Art-Net 4 DMX Receiver");
  ...
  printf("\nDMX receiver started, waiting for data... (Ctrl+C to stop)\n");
