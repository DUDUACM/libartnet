/*
 * dmx_server_tx_rx.c
 * Art-Net 4 DMX controller example (8-universe, input + output)
 *
 * Uses 2 joined nodes to support 8 universes.
 * Each node has 4 ports (Art-Net protocol limit), so 2 nodes = 8 ports.
 *
 * Usage: dmx_server_tx_rx [-i <bind_ip>] [-n <net>] [-s <subnet>] [-u <universe>]
 *                          [-c <channels>] [--no-chase]
...
static void print_nodes(artnet_node n) {
  artnet_node_list nl = artnet_get_nl(n);
  int count = artnet_nl_get_length(nl);
  if (count == 0) {
    printf("[Nodes] No nodes discovered\n");
    return;
  }

  printf("[Nodes] %d discovered:\n", count);
  artnet_node_entry entry = artnet_nl_first(nl);
  while (entry) {
    printf("  %d.%d.%d.%d  %-18s  Net %d Sub %d  Ports:",
...
static void print_usage(const char *prog) {
  ...
  printf("  --no-chase  Don't send DMX, just discover and receive\n");
}
...
int main(int argc, char *argv[]) {
  ...
  printf("Art-Net 4 DMX Controller (%d universes, %d nodes)\n", TOTAL_PORTS, NUM_NODES);
  printf("  Bind IP  : %s\n", ip ? ip : "auto");
  ...
  for (i = 0; i < NUM_NODES; i++) {
    ...
    artnet_set_node_type(nodes[i], ARTNET_SRV);
    artnet_set_style_code(nodes[i], ARTNET_ST_CONTROLLER);
    artnet_set_short_name(nodes[i], "LibArtNet Ctrl");
    artnet_set_long_name(nodes[i], "libartnet Art-Net 4 Controller");
  ...
  // Start all nodes (ARTNET_SRV auto-sends ArtPoll on start)
  for (i = 0; i < NUM_NODES; i++) {
  ...
  printf("\nController started, discovering nodes... (Ctrl+C to stop)\n\n");
  ...
  while (running) {
    ...
    // periodic ArtPoll to refresh node list
    int poll_tick = tick * FRAME_INTERVAL_MS;
    if (poll_tick - last_poll_tick >= POLL_INTERVAL_MS) {
      last_poll_tick = poll_tick;
      int ret = artnet_send_poll(nodes[0], NULL, ARTNET_TTM_AUTO);
      printf("[Poll] ret=%d (%s)\n", ret, ret == ARTNET_EOK ? "OK" : artnet_strerror());
      print_nodes(nodes[0]);
    }
